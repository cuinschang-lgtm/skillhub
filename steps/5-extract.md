...
B) Claude Sonnet 4.x
  ...
  ✅ 中文质量顶级
  ❌ 单本成本 ~10 倍 DeepSeek
  ⚠️ Anthropic Messages API 强制 max_tokens 字段（API 机制）；llm.py provider="anthropic" 内部硬编码 8192 默认值满足 API，仍不传 temperature
...
```bash
python3 ${CLAUDE_SKILL_DIR}/skeleton/extract.py \
  <chapters.json> \
  <output_dir> \
...
| 个别章节失败 | 单点 LLM 错误 | V1 默认 fail-fast，extract.py 抛异常 + 写 `_manifest.json`；用户可传 `--allow-partial` 接受缺章但 assemble/install 会拒绝 |
